                    ...
                    <div class="grid grid-flow-col gap-3">
                        ...
                        <div class=" bg-white    overflow-hidden shadow-sm sm:rounded-lg  col-span-4 ">
                            <div class="p-8">
                                ...
                                <div class="p-8">
                                    <form method="POST"
                                        action="{{ route('client_Register_update', Auth::user()->id) }}">
                                        @csrf
                                        <div>
                                            <x-label for="name" :value="__('name')" />
                                            <x-input id="name" class="block mt-1 w-full" type="text"
                                                name="name" :value="old('name')"  value="{{ $profile->name ?? ''}}" required autofocus />
                                        ...
                                        <div>
                                            ...
                                            <x-input id="age" class="block mt-1 w-full" type="text"
                                                name="age"  value="{{ $profile->age ?? ''}}" required autofocus />
                                        ...
                                        <div class="transititext-primary" data-te-toggle="tooltip"
                                         title="You can rank Risk using different levels, such as level 1 and level 2....">
                                            ...
                                            <x-label for="risk_rate" :value="__('risk rate')" />


                                            <x-input id="risk_rate" class="block mt-1 w-full" type="text"
                                                name="risk_rate" value="{{ $profile->risk_rate ?? ''}}" required autofocus />

                                        </div>
                                        <div   class="transititext-primary" data-te-toggle="tooltip"
                                         title="Add interested product type such as Equity, Structured Products">
                                            <x-label for="preferred_product" :value="__('preferred product')" />

                                            <x-input id="preferred_product" class="block mt-1 w-full" type="text"
                                                name="preferred_product"  value="{{ $profile->preferred_product ?? ''}}" required autofocus />
